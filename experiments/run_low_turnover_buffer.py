...
def records(data,seed=None):
 ...
 for day,group in data[data.TradingDay.isin(rebalance)].groupby("TradingDay"):
  ...
  selected=eligible[eligible.Symbol.astype(str).isin(target)].set_index(eligible[eligible.Symbol.astype(str).isin(target)].Symbol.astype(str)).loc[target].reset_index(drop=True);valid=selected.fwd5.notna();executed=set(selected.loc[valid,"Symbol"].astype(str));gross=float(selected.fwd5.fillna(0).sum()/TOP_N);benchmark=float(eligible.fwd5.dropna().mean());turnover=1.0 if not previous else 1-len(executed&set(previous))/TOP_N;cost=turnover*COST_PER_SIDE*2;rows.append({"date":pd.Timestamp(day),"gross_absolute":gross,"net_absolute":gross-cost,"benchmark":benchmark,"gross_active":gross-benchmark,"net_active":gross-benchmark-cost,"turnover":turnover,"missing_rate":1-float(valid.mean()),"executed_names":len(executed)});previous=target
