...
EXCLUDED={"半导体"}
def run(panel_path,states_path,positions_path,labels_path,filter_mode="base"):
 x=pd.read_csv(panel_path,usecols=["trade_date","sector","sector_index","benchmark_index","relative20","price_breadth"]);x["trade_date"]=pd.to_datetime(x.trade_date);x=x[~x.sector.isin(EXCLUDED)].copy();x["relative_rank"]=x.groupby("trade_date").relative20.rank(ascending=False,method="first");x=x.sort_values(["sector","trade_date"]);states=[];positions=[];labels=[]
 for sector,g in x.groupby("sector"):
  g=g.reset_index(drop=True).copy();g["relative_index"]=g.sector_index/g.benchmark_index;g["abs_high10_prev"]=g.sector_index.shift(1).rolling(10,min_periods=10).max();g["rel_high10_prev"]=g.relative_index.shift(1).rolling(10,min_periods=10).max();g["rel_low10_prev"]=g.relative_index.shift(1).rolling(10,min_periods=10).min();g["breakout"]=g.sector_index.gt(g.abs_high10_prev)&g.relative_index.gt(g.rel_high10_prev);g["entry_condition"]=g.breakout if filter_mode=="base" else g.breakout&g.relative_rank.le(3)&g.price_breadth.ge(.60);active=False;entry_index=None;position_id=None;hold=0
  for i,row in g.iterrows():
   ...
   states.append({"trade_date":row.trade_date,"sector":sector,"sector_index":row.sector_index,"benchmark_index":row.benchmark_index,"relative_index":row.relative_index,"relative20":row.relative20,"relative_rank":row.relative_rank,"price_breadth":row.price_breadth,"abs_high10_prev":row.abs_high10_prev,"rel_high10_prev":row.rel_high10_prev,"rel_low10_prev":row.rel_low10_prev,"breakout":bool(row.breakout),"entry_condition":bool(row.entry_condition),"enter_signal":enter,"exit_signal":exit_,"active_at_close":active and not exit_,"position_id":position_id if active else "","hold_sessions":hold if active else 0,"exit_reason":reason})
   if exit_:
 ...
 pd.DataFrame(states).to_csv(states_path,index=False,date_format="%Y-%m-%d");pd.DataFrame(positions).to_csv(positions_path,index=False,date_format="%Y-%m-%d");pd.DataFrame(labels).to_csv(labels_path,index=False,date_format="%Y-%m-%d");return {"filter_mode":filter_mode,"sectors":x.sector.nunique(),"states":len(states),"positions":len(positions),"trend_labels":len(labels),"states_path":states_path,"positions_path":positions_path,"labels_path":labels_path}
def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument("--panel",default="overall/a-share-sector-daily-2017-2026.csv");p.add_argument("--states",default="overall/a-share-sector-trend-states.csv");p.add_argument("--positions",default="overall/a-share-sector-trend-positions.csv");p.add_argument("--labels",default="overall/a-share-sector-month-trend-labels.csv");p.add_argument("--filter-mode",choices=["base","leader_breadth"],default="base");a=p.parse_args(argv);print(json.dumps(run(a.panel,a.states,a.positions,a.labels,a.filter_mode),ensure_ascii=False,indent=2));return 0
