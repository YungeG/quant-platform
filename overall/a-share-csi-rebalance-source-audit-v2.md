...
附件解析也已通过：`experiments/parse_csi_rebalance_attachments.py`从PDF/XLSX规范化出668行调入调出记录，沪深300为28进/28出，中证500为100进/100出，中证1000为206进/206出，未解析条目为0。规范化证据位于`a-share-csi-rebalance-2024-normalized.csv`；该小样本仍明确标记`backtest_ready=false`。
