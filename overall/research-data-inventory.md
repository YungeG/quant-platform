...
D004's diagnostic files were originally recorded under `platform/research/evidence/a-share-market-regime-20260921/`, including `public-full-v2/prices.csv` and `calendar.csv`. On **2026-09-22**, the user authorized direct deletion of all **31** untracked files in that dated directory, including smoke/failed captures and saved diagnostic replays. The working copies and their now-empty dated directory have been removed; no new data archive was created. Historical results remain documented, but this worktree no longer contains their exact replay inputs.

The earlier dependency-alignment check recorded a verified 84-file recovery archive at the then-current path `/home/ygguo/agent-projs/ai-crypt/.dependency-alignment-safety-20260921/platform-untracked.tar.gz`. That is a historical recovery record, not an active dataset location or Backtest publication. Existing backups were not changed or newly recovery-verified by this deletion; see the [directory-retirement follow-up](../implementation/dependency-alignment-20260921.md#worktree-retirement-follow-up) for their later organization. No data was reacquired or research rerun.
