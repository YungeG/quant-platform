...
## Final directory cleanup closure — 2026-09-22

The earlier follow-up records the state before the separately authorized final cleanup. These later operations do not change its historical verification results or grant any trading/data-acquisition authority.

- **Stale registrations:** all ten absent temporary worktrees in the stable `platform/backtest` repository were pruned after a complete `.git` archive (objects, refs, indexes and reflogs), exact dry-run allowlist comparison, and confirmation that every recorded HEAD had a retained branch/tag. All named refs, checked-out HEADs and working-file state remained unchanged; connectivity checks passed. No branches were deleted and no Git garbage collection ran. Recovery and checksums: `/srv/bcache-8t/ygguo/ai-crypt-backups/tail-cleanup-20260922-CtbsOS/CLEANUP-RECEIPT.md`.
- **Caches:** none of the eleven later cache candidates were removed. The occupancy check remained inconclusive because the protected `sd-pam` and `sshd-session` descriptor directories were inaccessible. No privilege escalation or process termination was attempted. This is a retained limitation, not a passing occupancy check.
- **Legacy repositories:** `crypto-quant-core`, `crypto-quant-platform`, and `pi-crypto-agents` were deleted only after their complete histories and current non-cache working files, including existing modifications, were actually restored and compared. Original archives remain under the `organization-20260921-hW0Sw8` backup root above; fresh metadata and restore checks are under `/srv/bcache-8t/ygguo/ai-crypt-backups/legacy-retirement-20260922-drjlnB/`. Its `RETIREMENT-RECEIPT.md` and `PACKAGE-REMOVAL-RECEIPT.md` record the two steps.
- **Project Agent configuration:** the user separately approved removing exactly `../../pi-crypto-agents` from `platform/.pi/settings.json` before deleting that package directory. The other two package entries and all other settings remained unchanged. Before/after configuration copies were preserved. This local file is Git-ignored; it was not force-added or committed. No current session was explicitly restarted or hot-reloaded, and already-loaded resources were not claimed to be hot-unloaded.
- **Retained work:** the main Platform, KORU and portfolio workspaces, the two A-share data workspaces, root evidence archives, previous recovery directories and `backtest` symlink remain. No unfinished source or research data was discarded. The main working tree's existing source/document changes were left for a separate, function-grouped review rather than swept into the cleanup commit.

These are local recovery copies on a separate filesystem, not offsite backups or Platform-published research evidence. Directory cleanup is closed; subsequent work is limited to reviewing and versioning independently validated file groups, with incomplete work and raw evidence retained.
